...
class BinFile:
    ...
    def read_header(cls, path):
        ...
        with open(path, mode='rb') as input_file:
            ...
            # Read number of npy_stack_frames.
    ...
    def __init__(self, path, frame_xsize, frame_ysize, rig_id, nb_images=0, mode='r'):

        self._path = path
        self._mode = mode

        assert rig_id == 1 or rig_id == 2 or rig_id == 3, f"unknown rig_id: {rig_id}"
        self._rig_id = rig_id

        if self._rig_id == 1:  # pixels
            self._max_dimension_x = 768  # might be bigger for one dimension but to check pixel size is 3.5 microns
            self._max_dimension_y = 768
            self._polarity = -1
            self._transformation = "90 deg counter-clockwise rotation and vertical flip"
        elif self._rig_id == 2:
            ...
            self._max_dimension_y = 768
            self._polarity = -1
            self._transformation = "horizontal flip"

        if self._mode == 'r':
            header = self.read_header(self._path)
            self._nb_images = header['nb_images']
            assert header['xsize'] <= self._max_dimension_x, f"image is too big on x axis for RIG {self._rig_id} ({header['xsize']} > {self._max_dimension_x}. "
            self._frame_xsize = header['xsize']
            assert header['ysize'] <= self._max_dimension_y, f"image is too big on y axis for RIG {self._rig_id} ({header['ysize']} > {self._max_dimension_y}. "
    ...
    def transform_frame_to_read(self, frame):
        """transforms a frame to correct for rotations, symetries, and polarity changes caused by the optical path of RIGs"""

        # Compensate for reverse polarity if needed
        if self._polarity == -1:
            frame = 1 - frame
        # Compensate for rotations/symmetries if needed
        if self._transformation == "90 deg counter-clockwise rotation and vertical flip":
            frame = np.flipud(np.rot90(frame, k=3))
        elif self._transformation == "horizontal flip":
            frame = np.fliplr(frame)

        # if self._rig_id == 3:
        #     frame = 1 - frame   # reverse polarity if necessary
        #     frame = np.fliplr(frame)  # compensate rotations/symetries
        # elif self._rig_id == 2:
        #     frame = np.flipud(np.rot90(frame))   # compensate rotations/symetries
        # elif self._rig_id == 1:
        #     frame = 1 - frame
        #     frame = np.fliplr(frame)
    ...
    def read_frame(self, frame_nb, verbose=0):
        ...
        frame_data = self.transform_frame_to_read(frame_data)
    ...
    def transform_frame_to_append(self, frame):
        """transforms a frame to correct for rotations, symmetries, and polarity changes caused by the optical path of RIGs"""
        ...
        if self._polarity == -1:
            ...
            # Compensate for rotations/symmetries if needed
        if self._transformation == "90 deg counter-clockwise rotation and vertical flip":
            frame = np.rot90(np.flipud(frame))
        elif self._transformation == "horizontal flip":
            frame = np.fliplr(frame)

        # if self._rig_id == 3:
        #     frame = 1 - frame   # reverse polarity
        #     frame = np.fliplr(frame)   # compensate rotations/symetries
        # elif self._rig_id == 2:
        #     frame = np.rot90(np.flipud(frame), k=3)   # compensate rotations/symetries
        # elif self._rig_id == 1:
        #     frame = 1 - frame
        #     frame = np.fliplr(frame)
    ...
    def append_frame(self, frame):
        ...
        if isinstance(frame, bytes):
            ...
            frame = self.transform_frame_to_append(frame)
