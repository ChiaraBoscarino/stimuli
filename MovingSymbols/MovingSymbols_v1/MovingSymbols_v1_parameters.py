...
pixel_size = 3.5  # in Âµm per pixel
...
symbol_sizes_um = np.array([int(x*SA_sq_dim*pixel_size) for x in normalized_symbol_sizes])  # Âµm
symbol_color = 1
background_color = 0
...
symbol_speeds_um = [int(x*pixel_size) for x in symbol_speeds]  # Âµm/s
...
>> Generating Moving Symbols stimulus (Version ID MovingSymbols_v1)
	...
	- Frame size: 2688.0x2688.0 µm (768x768 px)

	- Symbols: ['F']
	- Symbol sizes: [376] µm ([107] px)

	- Symbol trajectories: 1 ([6] points)
	- Symbol speeds: [175] µm/s ([50] px/s)
